# https://github.com/discourse/discourse/blob/master/lib/freedom_patches/active_record_base.rb

class ActiveRecord::Base
  def self.select_sql(*args)
  ...
  def self.exec_sql(*args)
    conn = ActiveRecord::Base.connection
    sql = ActiveRecord::Base.send(:sanitize_sql_array, args)
    #conn.raw_connection.exec(sql) # raw_connection doesn't log query
    conn.execute(sql)
  end
